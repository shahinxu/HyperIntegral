...
"""
function this(X::Matrix{Float64}, Y::Matrix{Float64}, ooi::Vector{Int64}, dmax::Int64, λ::Float64=.1, ρ::Float64=1., niter::Int64=10; apply_hard_threshold::Bool=false)
	...
	# Running SINDy / ridge coefficients.
	# By default we keep continuous scores (no hard thresholding) for downstream AUC evaluation.
	coeff, err = mySINDy(θ,Y,λ,ρ,niter; apply_hard_threshold=apply_hard_threshold)
	relerr = err/norm(Y,1)
...
"""
function this_filter(X::Matrix{Float64}, Y::Matrix{Float64}, ooi::Vector{Int64}, dmax::Int64, par_keep::Union{Int64,Float64}, λ::Float64=.1, ρ::Float64=1., niter::Int64=10; apply_hard_threshold::Bool=false)
	if size(X) != size(Y)
		@info "Dimensions of states and derivatives do not match."
		return nothing
	end
	...
	for i in 1:n
		...
		if isempty(id)
			...
			coef,er = mySINDy(θ,Y[[i,],:],λ,ρ,niter; apply_hard_threshold=apply_hard_threshold)
			ids[i] = id
		end
...
"""
function mySINDy(θ::Matrix{Float64}, Y::Matrix{Float64}, λ::Float64=.1, ρ::Float64=1., niter::Int64=10; apply_hard_threshold::Bool=true)
