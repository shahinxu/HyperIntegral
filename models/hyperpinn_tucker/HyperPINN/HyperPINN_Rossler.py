...
parser.add_argument('--tucker_rank', type=int, default=8)
...
results_base = os.environ.get('HYPERPINN_RESULTS_ROOT', os.path.join('results', 'hyperpinn_tucker'))
results_dir = os.path.join(
    results_base,
    'rossler',
    f'tucker_rank_{tucker_rank}',
    f'order_{max_order}',
...
print(f'Tucker analytic mode enabled: rank={tucker_rank}, max_order={max_order}')


def _save_true_hyperedge_figures(results_dir, N, true_2edges, true_3edges, name_prefix='true'):
...
def summarize_topology_stats(model):
    topo_param_names = ['tucker_raw_u2', 'tucker_raw_core2']
    if model.max_order >= 3:
        topo_param_names.extend(['tucker_raw_u3', 'tucker_raw_core3'])
...
for epoch in range(epochs):
    ...
    if epoch % 500 == 0:
        ...
        print(
            ...
            f"  Factor penalty: {sparsity_info['l1_tucker_factor_penalty']:.6f}"
        )
        ...
        for node_idx in range(N):
            for coord_idx in range(3):
                ...
                ax.plot(t_eval, X_train[:, node_idx + coord_idx * N], 'o', label='Data', markersize=4, alpha=0.7, color='blue')
                ax.plot(t_eval, X_pred[:, node_idx + coord_idx * N], '-', label='NN prediction', linewidth=1, alpha=0.8, color='red')
