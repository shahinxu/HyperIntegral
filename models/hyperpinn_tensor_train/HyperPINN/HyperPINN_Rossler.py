...
results_base = os.environ.get('HYPERPINN_RESULTS_ROOT', os.path.join('results', 'hyperpinn_tensor_train'))
...
print(f'TT analytic mode enabled: rank={tt_rank}, max_order={max_order}')
...
def _save_true_hyperedge_figures(results_dir, N, true_2edges, true_3edges, name_prefix='true'):
    ...
    for ax, order, true_list in zip(axes, orders, true_lists):
        ...
        for idx, e in enumerate(true_list):
            ...
            if len(nodes) == 2:
                i_idx, j_idx = nodes
                ax.plot([pos[i_idx][0], pos[j_idx][0]], [pos[i_idx][1], pos[j_idx][1]], color=color, linewidth=2.0, alpha=0.9)
            else:
