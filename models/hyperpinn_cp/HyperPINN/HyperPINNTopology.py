...
class HyperPINNTopology(nn.Module):
    def __init__(
        ...
        self.cp_raw_u2 = nn.Parameter(0.02 * torch.randn(N, cp_rank))
        self.cp_raw_lambda2 = nn.Parameter(0.02 * torch.randn(cp_rank))
        if max_order >= 3:
            self.cp_raw_u3 = nn.Parameter(0.02 * torch.randn(N, cp_rank))
            self.cp_raw_lambda3 = nn.Parameter(0.02 * torch.randn(cp_rank))
        else:
            self.cp_raw_u3 = None
